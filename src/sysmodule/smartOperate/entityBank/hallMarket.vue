<template>
  <div class="hallMarket-style">
    ...
    <el-row :gutter="15">
      <el-col :span="12">
        <div class="wave-content">
          <wave-chart v-if="waveMarketingCustomer&&waveMarketingCustomer.count!=null"
                      :chart-id="initData[0].waveChartId"
                      :chart-name="initData[0].chartName"
                      :chart-desc="initData[0].chartDesc"
                      :chart-img="initData[0].waveChartImg"
                      :wave-data="waveMarketingCustomer" />
          <occupied v-else
                    explain='暂无营销数据~~' />
        </div>
      </el-col>
      <el-col :span="12">
        <div class="wave-content">
          <wave-chart v-if="wavePotentialCustomer&&wavePotentialCustomer.count!=null"
                      :chart-id="initData[1].waveChartId"
                      :chart-name="initData[1].chartName"
                      :chart-desc="initData[1].chartDesc"
                      :chart-img="initData[1].waveChartImg"
                      :wave-data="wavePotentialCustomer" />
          <occupied v-else
                    explain='暂无营销数据~~' />
        </div>
      </el-col>
    ...
    <el-row :gutter="15">
      <el-col :span="12">
        <div class="wave-content">
          <wave-chart v-if="waveMarketingProduct&&waveMarketingProduct.count!=null"
                      :chart-id="initData[2].waveChartId"
                      :chart-name="initData[2].chartName"
                      :chart-desc="initData[2].chartDesc"
                      :chart-img="initData[2].waveChartImg"
                      :wave-data="waveMarketingProduct" />
          <occupied v-else
                    explain='暂无营销数据~~' />
        </div>
      </el-col>
      <el-col :span="12">
        <div class="wave-content">
          <wave-chart v-if="wavePurchasesRegistered&&wavePurchasesRegistered.count!=null"
                      :chart-id="initData[3].waveChartId"
                      :chart-name="initData[3].chartName"
                      :chart-desc="initData[3].chartDesc"
                      :chart-img="initData[3].waveChartImg"
                      :wave-data="wavePurchasesRegistered" />
          <occupied v-else
                    explain='暂无营销数据~~' />
        </div>
      </el-col>
    ...
    <el-row>
      <div class="market-type-style">
        <el-col :span="6">
          <div @click="marketTypeEvent('marketingCustomer')"
               class="market-init"
               :class="[initindex=='marketingCustomer'?'market-type':'']">营销客户数</div>
        </el-col>
        <el-col :span="6">
          <div @click="marketTypeEvent('potentialCustomer')"
               class="market-init"
               :class="[initindex=='potentialCustomer'?'market-type':'']">潜在客户登记数</div>
        </el-col>
        <el-col :span="6">
          <div @click="marketTypeEvent('marketingProduct')"
               class="market-init"
               :class="[initindex=='marketingProduct'?'market-type':'']">营销产品次数</div>
        </el-col>
        <el-col :span="6">
          <div @click="marketTypeEvent('purchasesRegistered')"
               class="market-init"
               :class="[initindex=='purchasesRegistered'?'market-type':'']">预约购买登记数</div>
        </el-col>
      </div>
    </el-row>
    <el-row :gutter="15">
      <el-col :span="12">
        <div class="wave-content">
          <entity-ranking-chart v-if="hallMarketRankingEntityData != null && hallMarketRankingEntityData.length > 0"
                                chart-id="firstRank"
                                :chart-name="entityInitData[initindex].chartName"
                                :chart-desc="entityInitData[initindex].chartDesc"
                                :branch-data="hallMarketRankingEntityData" />
          <occupied v-else
                    explain='暂无排名数据~~' />
        </div>
      </el-col>
      <el-col :span="12">
        <div class="wave-content-out">
          <entity-wave-chart v-if="hallMarketEntityData!== undefined && hallMarketEntityData.length > 0"
                             chart-id="firstWave"
                             :chart-name="entityInitData[initindex].chartName"
                             :chart-desc="entityInitData[initindex].chartDesc"
                             :chart-img="first"
                             :wave-data="hallMarketEntityData[0]" />
          <occupied v-else
                    explain='暂无营销数据~~' />
        </div>
      </el-col>
    ...
    <div v-if="hallMartetOtherData!=null&&hallMartetOtherData.length>0">
      <el-row :gutter="15"
              v-for="(item,index) in hallMartetOtherData"
              :key="index">
        <el-col :span="12">
          <div class="wave-content-out">
            <entity-wave-chart v-if="null!=item[0]"
                               :chart-id="'hallMartetOtherData'+index"
                               :chart-name="entityInitData[initindex].chartName"
                               :chart-desc="entityInitData[initindex].chartDesc"
                               :chart-img="index*2+2"
                               :wave-data="item[0]" />
        ...
        <el-col :span="12"
          ...
          <div class="wave-content-out">
            <entity-wave-chart v-if="null!=item[1]"
                               :chart-id="'hallMartetOtherData1'+index"
                               :chart-name="entityInitData[initindex].chartName"
                               :chart-desc="entityInitData[initindex].chartDesc"
                               :chart-img="index*2+3"
                               :wave-data="item[1]" />
