    ...
    <el-row :gutter="15" v-for="(item,index) in initData" :key="item.waveChartId">
      <el-col :span="12">
        <div class="wave-content">
          <wave-chart v-if="hallMarketData && hallMarketData[index] && hallMarketData[index].count!=null" :chart-id="item.waveChartId" :chart-name="item.chartName" :chart-desc="item.chartDesc" :chart-img="item.waveChartImg" :wave-data="hallMarketData[index]"/>
          <occupied v-else explain='暂无营销数据~~'/>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="wave-content">
          <branch-ranking-chart v-if="hallMarketRankingData && hallMarketRankingData[index] && hallMarketRankingData[index].count!=null" :chart-id="item.branchRankingChartId" :chart-desc="item.chartDesc" :chart-name="item.chartName"  :branch-data="hallMarketRankingData[index]"/>
