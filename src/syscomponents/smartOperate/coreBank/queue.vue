<template>
  <div class="queue-page-content">
    ...
    <el-row :gutter="15">
      <el-col :span="8" v-for="(item, index) in queueTimeByCustTypeList" :key="index">
        <div class="custerm-type-content">
          <customer-type-chart :line-chart-id="'coreCustomerLineChartId'+index" :circle-chart-id="'coreCustomerCircleChartId'+index" :customer-data="item"/>
        </div>
      </el-col>
    </el-row>
    <el-row>
    ...
    <el-row>
      <el-col :span="24">
        <div class="entity-content">
          <time-frame-chart chart-id="coreTimeFrameId" :chart-data="queueTimeDomainList"/>
        </div>
      </el-col>
    </el-row>
  </div>
  ...
  methods: {
    requestQueueDetail () {
      const params = {
        ...
        dateType: this.$store.state.timeRange,
        timeValue: this.$store.state.timeDate
      }
