...
export default {
  // 服务质量波浪图组件option
  entityMonthOption (xData, queueTimerData, averageTimerData, textname, avgname, rateData) {
    return {
      'grid': {
        'left': '15',
        'right': '20',
  ...
  // 服务质量排名柱状图组件
  entityBranchBarOption (xData, branchWaitData, defaultData, avgData, rateData, rankData, textname) {
    return {
      'color': ['#258AFF'],
      'title': {
        'text': textname + '排名',
        ...
        formatter: function (params) {
          ...
          params.forEach(function (item) {
            ...
            case 0:
              break
            case 1:
              result += item.name + ' : ' + (item.value).toFixed(2) + '%' + '</br>'
              break
            case 2:
              break
            case 3:
