<template>
  <div class="height-100" v-if="chartData!=null&&chartData.timeGroupCustFlowList!=null&&chartData.timeGroupCustFlowList.length>0" style="padding:10px">
    <el-row :gutter="10" class="height-23">
      <el-col :span="14">
        <div class="top-receive-count">
          <img :src="custFlowImg" class="cust-flow-img">
          <div class="timer-style">
            <font class="timer">{{chartData.receiveCount}}</font>
            <font class="timer-explain">接待客户数&nbsp;&nbsp;(人)</font>
          </div>
        </div>
      </el-col>
      <el-col :span="10">
        <div class="top-other">
          <div class="contrast-average">
            <font class="float-value" :class="[rateDataColor]">
              {{showRateData}}
              <img :src=" parseFloat(chartData.rate)>0?arrowsUp:arrowsDown" v-if="rateDataColor"/>
            </font>
            <font class="average-value" v-if="bankLevel != 'core'">{{chartData.totalAvg}}</font>
          </div>
          <div class="contrast-average">
            <font>环比</font>
            <font v-if="bankLevel != 'core'">全行平均&nbsp;&nbsp;(人)</font>
          </div>
        </div>
      </el-col>
    </el-row>
    <el-row class="height-77">
      <el-col :span="24" class="height-100">
        <div id="chartsId" class="height-100"></div>
      </el-col>
    </el-row>
  </div>
   <occupied v-else height='100%' explain="暂无接待客户数数据"/>
</template>

<script>
import echarts from 'echarts'
import options from './chartOptions'
import occupied from '@/components/Occupied/occupied'
import base from '@/common/base'

export default {
  name: 'customerNumChart',
  props: {
    chartData: {
      type: Object
    }
  },
  components: { occupied },
  data () {
    return {
      custFlowImg: require('@/assets/custflow_icon.png'),
      arrowsUp: require('@/assets/arrows_up.png'),
      arrowsDown: require('@/assets/arrows_down.png'),
      bankLevel: this.$store.state.operate.bankLevel
    }
  },
  mounted () {
    if (this.chartData != null) {
      this.initChart()
    }
  },
  methods: {
    initChart () {
      if (!(document.getElementById('chartsId'))) return
      const chart = echarts.init(document.getElementById('chartsId'))
      var xData = []
      var curList = []
      var averageList = []
      var rateList = []
      // 设置xAxis的值
      this.chartData.timeGroupCustFlowList.map(function (item) {
        xData.push(item.time)
        curList.push({ name: item.time, value: item.value })
        averageList.push({ name: item.time, value: item.totalAvg })
        if (!item.rate && item.rate !== 0) {
          rateList.push('noRate')
        } else {
          rateList.push(item.rate)
        }
      })

      var mOption = ''
      if (this.bankLevel === 'core') {
        const showDataCore = [{ 'name': '接待客户数', 'icon': 'circle' }]
        mOption = options.customerNumOption(base.timerShaft(xData), curList, [], showDataCore, rateList)
      } else {
        const showData = [{ 'name': '接待客户数', 'icon': 'circle' }, { 'name': '全行平均', 'icon': 'image:// data:image/png;base64,' }]
        mOption = options.customerNumOption(base.timerShaft(xData), curList, averageList, showData, rateList)
      }

      chart.setOption(mOption, true)
    },
    calFontColor (data) {
      if (data > 0) {
        return 'font-color-red'
      } else if (data < 0) {
        return 'font-color-blue'
      } else {
        return 'font-color-before'
      }
    }
  },
  watch: {
    'chartData.timeGroupCustFlowList': {
      handler (value) {
        this.$nextTick(function () {
          if (value != null) this.initChart()
        })
      },
      deep: true
    }
  },
  computed: {
    // 计算环比 如果环比不返回代表上一月为0，显示‘----’
    showRateData: function () {
      if (this.chartData.rate || this.chartData.rate === 0) {
        return (this.chartData.rate * 100).toFixed(2) + '%'
      } else {
        return '----'
      }
    },
    rateDataColor: function () {
      if (this.chartData.rate && this.chartData.rate !== 0) {
        if (this.chartData.rate > 0) {
          return 'font-color-red'
        } else {
          return 'font-color-blue'
        }
      } else {
        return ''
      }
    }
  }
}
</script>

<style rel="stylesheet/scss" lang="scss" scoped>
.top-receive-count {
  display: flex;
  flex-direction: row;
  .cust-flow-img {
    height: 40px;
    width: 40px;
    align-self: center;
  }
  .timer-style {
    display: flex;
    flex-direction: column;
    margin-left: 15px;
    .timer {
      display: flex;
      align-content: flex-start;
      font-size: 26px;
      color: #004e97;
      letter-spacing: 0;
    }
    .timer-explain {
      font-size: 12px;
      color: #9b9b9b;
      letter-spacing: 0;
    }
  }
}
.top-other {
  display: flex;
  flex-direction: column;
  height: 47px;
  justify-content: flex-end;
  .contrast-average {
    display: flex;
    justify-content: space-between;
    flex-direction: row;
    flex-shrink: 1;
    font {
      font-size: 12px;
      color: #9b9b9b;
      letter-spacing: 0;
    }
    .float-value {
      font-size: 18px;
      color: #004e97;
      font-weight: 600;
      img{
        vertical-align: middle;
      }
    }
    .average-value {
      font-size: 18px;
      color: #004e97;
      font-weight: 600;
    }
    .font-color-red {
      color: #E62112;

    }
    .font-color-blue {
      color: #50E3C2;
    }
  }
}
.month-chart {
  height: 100%;
  width: 100%;
}
.height-23 {
  height: 23%;
}
.height-77 {
  height: 77%;
}
.height-100 {
  height: 100%;
}
</style>

