<template>
  <div class="height-100" v-if="monthData&&monthData.distributionList&&monthData.distributionList.length>0">
    <el-row :gutter="10" class="height-23">
      <el-col :span="14">
        <div class="moth-chart-queue-wait-timer-style">
          <img :src="queueImg" class="queue-img">
          <div class="timer-style">
            <font class="timer">{{monthData.value}}</font>
            <font class="timer-explain">排队等待时长&nbsp;&nbsp;(分钟)</font>
          </div>
        </div>
      </el-col>
      <el-col :span="10">
        <div class="moth-chart-contrast-average-content">
          <div class="contrast-average">
            <font class="float-value" :class="[rateDataColor]">
              {{showRateData}}
              <img :src=" parseFloat(monthData.rate)>0?arrowsUp:arrowsDown" v-if="rateDataColor"/>
            </font>
            <font class="average-value" v-if="bankLevel != 'core'">{{monthData.totalAvg}}</font>
          </div>
          <div class="contrast-average">
            <font>环比</font>
            <font v-if="bankLevel != 'core'">全行平均&nbsp;&nbsp;(分钟)</font>
          </div>
        </div>
      </el-col>
    </el-row>
    <el-row class="height-77">
      <el-col :span="24" class="height-100">
        <div id="chartsId" class="month-chart"></div>
      </el-col>
    </el-row>
  </div>
  <occupied v-else height='100%' explain="暂无排队等待时长线形图数据"/>
</template>

<script>
import echarts from 'echarts'
import options from './chartOptions'
import occupied from '@/components/Occupied/occupied'
import base from '@/common/base'

export default {
  name: 'monthChart',
  props: { monthData: Object },
  components: { occupied },
  data () {
    return {
      queueImg: require('@/assets/queue_icon.png'),
      arrowsUp: require('@/assets/arrows_up.png'),
      arrowsDown: require('@/assets/arrows_down.png'),
      bankLevel: this.$store.state.operate.bankLevel
    }
  },
  mounted () {
    this.initChart()
  },
  methods: {
    initChart () {
      if (!(document.getElementById('chartsId'))) return
      const chart = echarts.init(document.getElementById('chartsId'))
      const xData = []
      const curList = []
      const averageList = []
      const rateData = []
      // 设置xAxis的值
      this.monthData.distributionList.map(function (item) {
        xData.push(item.time)
        curList.push(item.value)
        averageList.push(item.totalAvg)
        rateData.push(item.rate !== undefined ? (parseFloat(item.rate) * 100).toFixed(2) : 'null')
      })

      var mOption = ''
      if (this.bankLevel === 'core') {
        const showDataCore = [{ 'name': '排队等待时长', 'icon': 'circle' }]
        mOption = options.entityMonthOption(base.timerShaft(xData), curList, [], showDataCore, rateData)
      } else {
        const showData = [{ 'name': '排队等待时长', 'icon': 'circle' }, { 'name': '全行平均', 'icon': 'image:// data:image/png;base64,' }]
        mOption = options.entityMonthOption(base.timerShaft(xData), curList, averageList, showData, rateData)
      }
      chart.setOption(mOption, true)
    }
  },
  watch: {
    'monthData.distributionList': {
      handler (value) {
        this.$nextTick(function () {
          // console.log(value)
          if (value && value.length > 0) this.initChart()
        })
      },
      deep: true
    }
  },
  computed: {
    // 计算环比 如果环比不返回代表上一月为0，显示‘----’
    showRateData: function () {
      if (this.monthData.rate || this.monthData.rate === 0) {
        return ((this.monthData.rate) * 100).toFixed(2) + '%'
      } else {
        return '----'
      }
    },
    rateDataColor: function () {
      if (this.monthData.rate && this.monthData.rate !== 0) {
        if (this.monthData.rate > 0) {
          return 'font-color-red'
        } else {
          return 'font-color-blue'
        }
      } else {
        return ''
      }
    }
  }
}
</script>

<style rel="stylesheet/scss" lang="scss" scoped>
.moth-chart-queue-wait-timer-style {
  display: flex;
  flex-direction: row;
  .queue-img {
    height: 40px;
    width: 40px;
    align-self: center;
  }
  .timer-style {
    display: flex;
    flex-direction: column;
    margin-left: 15px;
    .timer {
      display: flex;
      align-content: flex-start;
      font-size: 26px;
      color: #004e97;
      letter-spacing: 0;
    }
    .timer-explain {
      font-size: 12px;
      color: #9b9b9b;
      letter-spacing: 0;
    }
  }
}
.moth-chart-contrast-average-content {
  display: flex;
  flex-direction: column;
  height: 47px;
  justify-content: flex-end;
  .contrast-average {
    display: flex;
    justify-content: space-between;
    flex-direction: row;
    flex-shrink: 1;
    font {
      font-size: 12px;
      color: #9b9b9b;
      letter-spacing: 0;
    }
    .float-value {
      font-size: 18px;
      color: #004e97;
      font-weight: 600;
      img{
        vertical-align: middle;
      }
    }
    .average-value {
      font-size: 18px;
      color: #004e97;
      font-weight: 600;
    }
    .font-color-red {
      color: #E62112;

    }
    .font-color-blue {
      color: #50E3C2;
    }
  }
}
.month-chart {
  height: 100%;
  width: 100%;
}
.height-23{
  height: 23%;
}
.height-77{
  height: 77%;
}
.height-100{
  height: 100%;
}
</style>

