  ...
  <el-row>
    <el-col :span="24">
      <div class="trade-time-bar">
        ...
        <occupied v-else height='100%' explain="暂无高频交易图表数据"/>
      </div>
    </el-col>
  </el-row>
  <div v-if="anewArr&&anewArr.length>0">
    <el-row :gutter="20" v-for="(item,index) in anewArr" :key="index+20">
      <el-col :span="8">
        <div class="item-temple">
          <deal-type-module v-if="null!=item[0]" :deal-data="item[0]" :chart-id="'leftTradeId0'+index" tran-flag="1" :m-name="$parent.$options.name" @onReversalListen="onReversalListen" :curr-reversal-id="currReversalId"></deal-type-module>
        </div>
      </el-col>
      <el-col :span="8" :style="{display:null!=item[1]?'':'none'}">
        <div class="item-temple">
          <deal-type-module v-if="null!=item[1]" :deal-data="item[1]" :chart-id="'leftTradeId1'+index" tran-flag="1" :m-name="$parent.$options.name" @onReversalListen="onReversalListen" :curr-reversal-id="currReversalId"></deal-type-module>
        </div>
      </el-col>
      <el-col :span="8" :style="{display:null!=item[2]?'':'none'}">
        <div class="item-temple">
          <deal-type-module v-if="null!=item[2]" :deal-data="item[2]" :chart-id="'leftTradeId2'+index" tran-flag="1" :m-name="$parent.$options.name" @onReversalListen="onReversalListen" :curr-reversal-id="currReversalId"></deal-type-module>
        </div>
      </el-col>
    </el-row>
  </div>
  <occupied v-else height='400px' margin-top="15px" explain="暂无高频交易列表数据"/>
</div>
</template>
<script>
  ...
  methods: {
    ...
    requestHighFrequencyTrading () {
      ...
      this.$http.getTrend('highFrequencyTrading', params).then((result) => {
        ...
        this.highFrequencyTrade.tradeName = '高频交易业务总量  (笔)                  高频交易平均处理时长  (分钟)'
