<template>
  <div class="device-state">
    <div class="top-style">
      <el-row :gutter="20">
        <el-col :span="10">
          <div class="device-title-style customer-title">
            <b>对私普通</b>
          </div>
        </el-col>
        <el-col :span="14">
    ...
    <div v-if="this.chartsData1[0].total>0">
      <privateOrdinaryCustChart
        :chartsData="chartsData"
        :busyCustGatherInfoInCustType="busyCustGatherInfoInCustType"
        :timePonitResultDate="timePonitResultDate"
        :startEndTimePoint="startEndTimePoint"
      ></privateOrdinaryCustChart>
    </div>
...
import privateOrdinaryCustChart from '@/syscomponents/smartMonitor/monitor/busyDegree/charts/privateOrdinaryCustChart'
...
export default {
  ...
  components: {
    occupied, privateOrdinaryCustChart
  },
  mounted () {
  ...
  methods: {
    ...
    async getData () {
      ...
      try {
        ...
        me.windowID = chartsDatas.personalCustomGatherInfo.windows || []
        me.busyCustGatherInfoInCustType.avgWaitingTime = chartsDatas.personalCustomGatherInfo.busyCustGatherInfoInCustType.avgWaitingTime || 0
        me.busyCustGatherInfoInCustType.waitingCustomers = chartsDatas.personalCustomGatherInfo.busyCustGatherInfoInCustType.waitingCustomers || 0
        me.busyCustGatherInfoInCustType.waitingCustomersRate = chartsDatas.personalCustomGatherInfo.busyCustGatherInfoInCustType.waitingCustomersRate || 0
        me.$set(me.chartsData[0], 'waitTimeRate', parseFloat(chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.lessThanTenMinRate || 0).toFixed(2))
        me.$set(me.chartsData[1], 'waitTimeRate', parseFloat(chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.betweenTenAndTwentyMinRate || 0).toFixed(2))
        me.$set(me.chartsData[2], 'waitTimeRate', parseFloat(chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.betweenTwentyAndThirtyMinRate || 0).toFixed(2))
        me.$set(me.chartsData[3], 'waitTimeRate', parseFloat(chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.moreThanThirtyMinRate || 0).toFixed(2))
        me.$set(me.chartsData1[0], 'total', chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.totalCount || 0)
        me.$set(me.chartsData[0], 'waitTimeValue', chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.lessThanTenMinCount || 0)
        me.$set(me.chartsData[1], 'waitTimeValue', chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.betweenTenAndTwentyMinCount || 0)
        me.$set(me.chartsData[2], 'waitTimeValue', chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.betweenTwentyAndThirtyMinCount || 0)
        me.$set(me.chartsData[3], 'waitTimeValue', chartsDatas.personalCustomGatherInfo.busyWaitTimeDistribution.moreThanThirtyMinCount || 0)
      } catch (err) {
        this.$message({
          message: err.message,
          type: 'error'
    ...
    async getLineChartData () {
      ...
      try {
        ...
        const startEndTime = await services.api.monitor.monitorBusy.getOrgBusinessHours(this.branchId)
        const chartsDatas = await services.api.monitor.monitorBusy.getPhasedCustomerTypeBusyInfo(this.branchId, 1, startEndTime.endTime, startEndTime.startTime)
        self.timePonitResultDate = chartsDatas.personalCustomGatherInfo.timePhasedBusinessInfoList || []
    ...
    dataRecombination () {
      const self = this
      self.timePonitOriginalDate.map((item, index) => {
        self.startEndTimePoint.map((item1, index1) => {
          if (item1.endTime === item.endTime) {
            ...
            self.startEndTimePoint[index1]['applyCustomers'] = item.applyCustomers// 抽号数量
            self.startEndTimePoint[index1]['callCustomers'] = item.callCustomers// 叫号数量
          }
        })
      })
